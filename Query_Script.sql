...
/*Question 3.15*/
SELECT * FROM foodgroups WHERE foodgroups.id NOT IN (SELECT DISTINCT(ingredients.food_group_id) FROM ingredients JOIN requires ON foodgroups.id = requires.ingredient_id
